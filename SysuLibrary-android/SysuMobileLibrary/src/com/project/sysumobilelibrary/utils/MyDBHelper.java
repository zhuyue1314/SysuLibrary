...
//		db.execSQL(sql);
		...
		sql = "create table "
				+ TB_SEARCHS
				+ "(search_str text, no integer, search_keyword text, doc_number text, primary key(search_str, search_keyword, no), foreign key(doc_number) references searchBooks(doc_number) on delete cascade on update cascade);";
		db.execSQL(sql);
	...
	// 查询
	public ArrayList<LikeBook> querySearchs(String search_str, String search_keyword, Integer no, Integer num){
		SQLiteDatabase db = getReadableDatabase();
		String sql = "select * from Searchs, SearchBooks where search_str=? and search_keyword=? and no>=? and Searchs.doc_number=SearchBooks.doc_number limit ?;";
		Cursor cursor = db.rawQuery(
				sql,
				new String[] {search_str, search_keyword, no.toString(), num.toString()});
		ArrayList<LikeBook> books = new ArrayList<LikeBook>();
		while (cursor.moveToNext()){
			LikeBook book = new LikeBook();
...
//	}
	...
	private int deleteSearchs(String search_str, Integer no) {
		SQLiteDatabase db = getWritableDatabase();
		String whereClause = "search_str = ? and no = ?";
		String[] whereArgs = { search_str, no.toString() };
